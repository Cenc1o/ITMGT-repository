...
for a in range(len(jsondata)):
    jsondata[str(a)] == str(jsondata[str(a)])

for x  in range(len(jsondata)):
    jsondata[str(x)] = jsondata[str(x)].upper()


for y in range(len(jsondata)):
    while True:
        if "<P>" in jsondata[str(y)]:
            jsondata[str(y)] = jsondata[str(y)].replace("<P>","")
            
        if "</P>" in jsondata[str(y)]:
            jsondata[str(y)] = jsondata[str(y)].replace("</P>","")
            
        if "<B>" in jsondata[str(y)]:
            jsondata[str(y)] = jsondata[str(y)].replace("<B>","")
            
        if "</B>" in jsondata[str(y)]:
            jsondata[str(y)] = jsondata[str(y)].replace("</B>","")
            
        if "<I>" in jsondata[str(y)]:
            jsondata[str(y)] = jsondata[str(y)].replace("<I>","")
            
        if "</I>" in jsondata[str(y)]:
            jsondata[str(y)] = jsondata[str(y)].replace("</I>","")
            
        if ("<P>"  not in jsondata[str(y)])and ("</P>"  not in jsondata[str(y)]) and ("<B>"  not in jsondata[str(y)]) and ("</B>"  not in jsondata[str(y)]) and ("<I>"  not in jsondata[str(y)]) and ("</I>"  not in jsondata[str(y)]):
            break
        else:
            continue

for b in range(len(jsondata)):
    for c in unnecessary_punctuation:
        if c in jsondata[str(b)]:
            jsondata[str(b)] = jsondata[str(b)].replace(c," ")
        else:
            continue
...
for e in range(len(jsondata)):
    list_words = jsondata[str(e)].split()
    for f in list_words:
        if len(f) >= 3:
            if f in wordcount_dictionary.keys():
               wordcount_dictionary[f] += 1
            else:
                wordcount_dictionary[f] = 1
...
for q in range(1, 18+ 1):
    for w in range(1, 16+1):
        for r in range(1, 20+ 1):
            ...
            if (optimal_Z == 344):# and ((1000*optimal_F + 600*optimal_R + 750*optimal_C) <= 18000) and ((500*optimal_F + 750*optimal_R + 300*optimal_C) <= 12000) and ((50*optimal_F + 45*optimal_R + 75*optimal_C) <= 1500):
                break
        if optimal_Z == 344:# and ((1000*optimal_F + 600*optimal_R + 750*optimal_C) <= 18000) and ((500*optimal_F + 750*optimal_R + 300*optimal_C) <= 12000) and ((50*optimal_F + 45*optimal_R + 75*optimal_C) <= 1500):
            break
    if optimal_Z == 344:# and ((1000*optimal_F + 600*optimal_R + 750*optimal_C) <= 18000) and ((500*optimal_F + 750*optimal_R + 300*optimal_C) <= 12000) and ((50*optimal_F + 45*optimal_R + 75*optimal_C) <= 1500):
  ...
  # the extra requirements makes the code not work for some reason
...
def Collatz(start_number):
    ...
    max_collatz = collatz_sequence[0]
...
def Collatz_winner(number_list):
    ...
    for x in number_list:
        ...
        new_collatz_length = len(collatz_sequence)
    
        for y in collatz_sequence:
            if y > new_max_collatz:
                new_max_collatz = y
            else:
                continue

        if new_max_collatz >= max_collatz and new_collatz_length >= collatz_length:
            max_collatz = new_max_collatz
            collatz_length = new_collatz_length
            winner = current_num

        
    if max_collatz == 0  and collatz_length == 0:
        return "none"
    
    return winner

Collatz_winner(range(75,180,9))
